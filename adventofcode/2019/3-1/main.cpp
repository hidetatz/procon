...
class Direction {
    ...
    std::tuple<int, int> Dir() const {
        if (direction == "U") return std::make_tuple(0, 1);
        if (direction == "D") return std::make_tuple(0, -1);
        if (direction == "R") return std::make_tuple(1, 0);
...
class Wire {
    ...
    std::vector<Point> intersections(Wire w) {
...
Wire newWire(std::vector<Direction> const dirs) {
    std::vector<Point> points;
...
int main() {
    ...
    std::cout << intersections.at(0).manhattanDistance() << std::endl;
}
